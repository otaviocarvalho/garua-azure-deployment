...
if [[ -z "$1" ]]; then
    ...
    parallel-ssh -h pssh_master_hosts.txt -t 600 -p 1 -l ocarval -x "-o StrictHostKeyChecking=no -i ../../credentials/id_rsa" bash /home/ocarval/go/src/grpc_poc/install_dependencies_cloud.sh
    ...
    parallel-ssh -h pssh_edge_hosts.txt -t 600 -p $1 -l ocarval -x "-o StrictHostKeyChecking=no -i ../../credentials/id_rsa" bash /home/ocarval/go/src/grpc_poc/install_dependencies_cloud.sh
fi
